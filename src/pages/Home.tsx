            ...
            {portfolioPreviewImages.map((image, index) => (
              <ScrollReveal key={index} delay={index * 0.05} direction="scale">
                <div className="relative group overflow-hidden rounded-lg aspect-square hover-lift">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-indigo-900/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-6">
                    <button className="glass text-white px-6 py-2 rounded-lg font-semibold transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                      View Details
                    </button>
                  </div>
                </div>
